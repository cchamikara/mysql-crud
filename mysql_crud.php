<?php
/*
 * @Customised by chamal
 */
require_once("$_SERVER[DOCUMENT_ROOT]/scraper_v3/config.php");
...
class Database{
	...
	// Function to SELECT from the database
	public function selectone($table, $rows = '*', $join = null, $where = null, $order = null, $limit = null){
		// Create query from the variables passed to the function
		$q = 'SELECT '.$rows.' FROM '.$table;
		if($join != null){
			$q .= ' JOIN '.$join;
	...
	private function tableExists($table){
		...
		$tablesInDb = $this->myconn->query('SHOW TABLES LIKE "'.$table.'"');
        if($tablesInDb){
